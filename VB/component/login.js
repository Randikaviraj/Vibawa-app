...
export default class Login extends ValidationComponent {
  ...
  handleSignin = () => {
    if (!this.state.email) {
      Alert.alert("Error", "To sign in,enter your email", [
        {
          text: "OK",
          onPress: () => console.log("OK Pressed in missing email"),
        },
      ]);
      return;
    }
    ...
    if (!this.state.password) {
      Alert.alert("Error", "To sign in,enter password", [
        {
          text: "OK",
          onPress: () => console.log("OK Pressed in missing password"),
        },
      ]);
      return;
    }
  ...
  componentDidUpdate() {
    if (this.state.responseStatus) {
      if (!this.state.response.statussignin && !this.state.response.network) {
        Alert.alert("Error", "Couldn't sign in check your network connection", [
          { text: "OK" },
        ]);
        this.setState({
          email: "",
      ...
      if (!this.state.response.statussignin && this.state.response.network) {
        Alert.alert(
          "Couldn' SignIn",
          "Couldn't sign in your entered password or email is wrong",
          [{ text: "OK" }]
        );
        this.setState({
          email: "",
  ...
  getSignIn = async (data) => {
    try {
      fetch("http://192.168.1.103:3330/user/login", {
        method: "POST",
        body: JSON.stringify({
          email: data.email,
      ...
      console.log("Error in login in login page..............." + e);
      this.setState({
        response: { statussignin: false, network: false },
        responseStatus: true,
      });
